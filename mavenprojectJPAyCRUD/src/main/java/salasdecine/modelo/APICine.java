...
import salasdecine.dao.DAOCliente;
import salasdecine.dao.HibernateUtil;
import salasdecine.dao.PeliculaJDBC;
...
public class APICine {
	...
	// NUEVO AGREGADO PARA QUE FUNCIONE EL PERSISTENCE, AHORA NO REGISTRA USANDO EL
	// METO API, SI NO EL DAO DE AQUI ABAJO
	private DAOCliente dao = new DAOCliente();

	// PARA PODER USAR LOS METODOS DE ALTA PELICULA Y CONSULTAR PELICULA
	private PeliculaJDBC peliculaJDBC = new PeliculaJDBC();
	...
	public void registrarCliente(String nombre, String email, String fechaNacimiento) {
		/*
		 * USABA ESTO PARA QUE REGISTRE USANDO CINE Y EL METODO DE CINE REGISTRAR
		 * CLIENTE /cine.registroCliente(nombre, email, fechaNacimiento);
		 */

		// USA PARA HACERLO POR DAO
		Cliente cliente = new Cliente(nombre, email, fechaNacimiento);
		dao.guardar(cliente);
	}

	/**
	 * METODO CONSULTAR CLIENTE PARA USARLO CON DATOS QUE AGREGABAS EN EL MOMENTO
	 * PERO SI CERRABAS ESOS DATOS SE BORRABAN
	 * 
	 * @param idCliente
	 * @return
	...
	public String[][] consultaClienteDAO(int id) throws NotFoundClienteException {

		Cliente cliente = dao.buscarPorID(id);
		if (cliente == null) {
			throw new NotFoundClienteException();
		}
		String[][] resultadoEncontrado = new String[1][4];
		resultadoEncontrado[0][0] = String.valueOf(cliente.getId_Cliente());
		resultadoEncontrado[0][1] = cliente.getNombre();
		resultadoEncontrado[0][2] = cliente.getEmail();
		resultadoEncontrado[0][3] = cliente.getFecha_nacimiento();
	...
	public String[][] listaClienteDAO() {
		ArrayList<Cliente> listaCliente = (ArrayList<Cliente>) dao.listaDeClientes();
		String[][] resultadoListaClientes = new String[listaCliente.size()][4];
		if (listaCliente.size() == 0) {
			throw new NullPointerException();
		}
		int fila = 0;
		for (Cliente v : listaCliente) {
			resultadoListaClientes[fila][0] = String.valueOf(v.getId_Cliente());
			resultadoListaClientes[fila][1] = v.getNombre();
			resultadoListaClientes[fila][2] = v.getEmail();
			resultadoListaClientes[fila][3] = v.getFecha_nacimiento();
	...
	/**
	 * METODO PARA LISTAR LOS CLIENTES EN VENDER ENTRADA, PERO SOLO ID Y NOMBRE
	 * @return
	 */
	public String[][] listaClienteDAOVenderEntrada(){
		ArrayList<Cliente> listaCliente = (ArrayList<Cliente>) dao.listaDeClientes();
		
		String[][] resultadoListaClientes = new String[listaCliente.size()][2];
		//SI LISTA VACIA SALE EXCEPCION
		if (listaCliente.size() == 0) {
			throw new NullPointerException();
		}
		int fila=0;
	...
	/**
	 * METODO NUEVO PARA USAR JDBC PARA INSERTAR DATOS DE LA PELICULA
	 * 
	 * @param titulo
	 * @param duracion
	 * @param genero
	 * @param clasificacionEdad
	 */
	public void registrarPeliculaJDBC(String titulo, int duracion, String genero, ClasificacionEdad clasificacionEdad) {
		peliculaJDBC.altaPelicula(titulo, duracion, genero, clasificacionEdad);
	}
	...
	// METODO PARA USAR EL LISTADO DE PELIOCULAS DE LA BASE DE DATOS CON JDBC
	public String[][] listaPeliculasJDBC() {

		String[][] resultadoDeBuscarPeliculas = peliculaJDBC.listaPeliculas();
		String[][] resultado = new String[resultadoDeBuscarPeliculas.length][2];

		if (resultadoDeBuscarPeliculas.length == 0) {
			throw new NullPointerException();
			// NO HABRIA RESULTADO
		}
		for (int fila = 0; fila < resultadoDeBuscarPeliculas.length; fila++) {

			resultado[fila][0] = String.valueOf(fila + 1);
			resultado[fila][1] = resultadoDeBuscarPeliculas[fila][0];// TENGO EL TITULO DE LA PELI
		}
		return resultado; // TENDRIA EL NUMERO Y LA PELICULA TITULO SOLO
	}
	...
	// METODO IGUAL QUE EL DE ARRIBA PERO ADAPTADO PARA EL JDBC
	public String[] numerosDePeliculasJDBC() {
		String[][] peliculas = peliculaJDBC.listaPeliculas(); // obtengo las pelis

		if (peliculas.length == 0) {
			throw new NullPointerException();
		}
		String[] resultado = new String[peliculas.length];
		for (int i = 0; i < peliculas.length; i++) {
			resultado[i] = String.valueOf(i + 1);
		}
		return resultado; // obtengo el array de numeros de peliculas que usare el el jcombo
	}
	...
	/**
	 * METODO USADO PARA QUE ME RETOME LA PELICULA POR INDICE DEE LA ANTIGUA FORMA
	 * 
	 * @param indice
	 * @return
	 */
	...
	/**
	 * MISMO METODO DE ARRIBA PERO ADAPTADO PARA JDBC
	 * @param indice
	 * @return
	 */
	public Pelicula getPeliculaPorIndiceJDBC(int indice) {
		String[][] peliculas = peliculaJDBC.listaPeliculas();
		if (indice < 0 || indice >= peliculas.length) {
			return null;
		}
		String titulo = peliculas[indice][0];
		return peliculaJDBC.buscarPorTitulo(titulo);
	}
	 ...
	 * CLIENTE
	 * USADO ANTES DE IMPLANTAR EL JPA Y JDBC
	 * @return
	 */
	...
	public String[] numeroClienteListaDAO() {
		List<Cliente> clientes = dao.listaDeClientes(); // obtengo los clientes
		if (clientes.size() == 0) {
			throw new NullPointerException();
		}
		...
		return resultado; // obtengo el array de numeros de clientes que usare el el jcombo
	}
	...
	/**
	 * METODO QUE CREAR EL ARRAY LIST CLIENTE Y LUEGO CON ESE CLIENTE COMPRUEBA LAS
	 * ENTRADAS QUE TIENE Y LAS VA GUARDANDO
	 * 
	 * @param indiceCliente
	 * @return
	 */
