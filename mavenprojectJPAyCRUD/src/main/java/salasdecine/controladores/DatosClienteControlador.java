...
public class DatosClienteControlador implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		DAOCliente dao = new DAOCliente(); //CREO EL OBJETO dao PARA RECUPERAR EL CLIENTE CON LA CONSULTA
		try {
			// TENEMOS EL VALOR DEL ID CLIENTE
			int idCliente = Integer.parseInt(vista.getIdCliente());
			System.out.println(idCliente);
			/* LO USABAMOS PARA QUE RECIBIERAMOS EL CLIENTE PERO DE LOS QUE SSE CREABAN AL INSTANTE
			 * ES DECIR SI PAUSABAS ELPROGRAMA E INICIABAS YA NO EXITE CLIENTE
			/resultadoCliente=api.consultaCliente(idCliente); //OBTENEMOS DEL API Y GUARDAMOS EL CLIENTE 
			*/
			resultadoCliente=api.consultaClienteDAO(idCliente);
